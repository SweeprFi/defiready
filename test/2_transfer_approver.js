...
contract("TransferApprover", async function () {
	before(async () => {
		...
		await sweep.mint(user1.address, TRANSFER_AMOUNT.mul(3))
		...
		transferApprover = await TransferApprover.deploy(sweep.address, whitelist.address);

		await sweep.setTransferApprover(transferApprover.address);
	});
	...
	it('reverts all transfers between non-valid users when validation method is ONLY_VALID', async () => {
		...
		await expect(sweep.connect(user1).transfer(user2.address, TRANSFER_AMOUNT))
	...
	it('Transfer sweep correctly between valid users when validation method is ONLY_VALID', async () => {
		// check validation method is ONLY_VALID
		expect(await transferApprover.validation_method()).to.equal(Const.ONLY_VALID);
		...
		expect(await sweep.balanceOf(user2.address)).to.equal(Const.ZERO);

		await sweep.connect(user1).transfer(user2.address, TRANSFER_AMOUNT)

		expect(await sweep.balanceOf(user2.address)).to.equal(TRANSFER_AMOUNT);
	});
	...
	it('reverts transfers when validation method is VALID_OR_NEW', async () => {
		...
		await expect(sweep.connect(user2).transfer(user3.address, TRANSFER_AMOUNT))
			.to.be.revertedWithCustomError(Sweep, 'TransferNotAllowed');
		...
		await expect(sweep.connect(user3).transfer(user4.address, TRANSFER_AMOUNT))
			.to.be.revertedWithCustomError(Sweep, 'TransferNotAllowed');

	});

	it('Transfer sweep correctly when validation method is ONLY_VALID', async () => {
		// check validation method is ONLY_VALID
		expect(await transferApprover.validation_method()).to.equal(Const.VALID_OR_NEW);
		...
		expect(await sweep.balanceOf(user3.address)).to.equal(Const.ZERO);

		await sweep.connect(user2).transfer(user3.address, TRANSFER_AMOUNT)

		expect(await sweep.balanceOf(user3.address)).to.equal(TRANSFER_AMOUNT);
		...
		expect(await sweep.balanceOf(user4.address)).to.equal(Const.ZERO);

		await sweep.connect(user3).transfer(user4.address, TRANSFER_AMOUNT)

		expect(await sweep.balanceOf(user4.address)).to.equal(TRANSFER_AMOUNT);
	});
	...
	it('revert transfers when owner sets user to INVALID', async () => {
		...
		await expect(sweep.connect(user4).transfer(user1.address, TRANSFER_AMOUNT))
			.to.be.revertedWithCustomError(Sweep, 'TransferNotAllowed');
	});
});
